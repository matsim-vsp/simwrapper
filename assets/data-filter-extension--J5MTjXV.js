import{s as L,y as Y,g as O}from"./layer-CsZTuiFD.js";import{L as P}from"./layer-extension-aMEhI4n_.js";const _=`uniform dataFilterUniforms {
  bool useSoftMargin;
  bool enabled;
  bool transformSize;
  bool transformColor;
...
${_}
${k}
`;function y(s){if(!s||!("extensions"in s))return{};const{filterRange:e=[-1,1],filterEnabled:t=!0,filterTransformSize:a=!0,filterTransformColor:i=!0,categoryBitMask:r}=s,l=s.filterSoftRange||e;return{...Number.isFinite(e[0])?{min:e[0],softMin:l[0],softMax:l[1],max:e[1]}:{min:e.map(o=>o[0]),softMin:l.map(o=>o[0]),softMax:l.map(o=>o[1]),max:e.map(o=>o[1])},enabled:t,useSoftMargin:!!s.filterSoftRange,transformSize:t&&a,transformColor:t&&i,...r&&{categoryBitMask:r}}}function B(s){if(!s||!("extensions"in s))return{};const e=y(s);if(Number.isFinite(e.min)){const t=Math.fround(e.min);e.min-=t,e.softMin-=t,e.min64High=t;const a=Math.fround(e.max);e.max-=a,e.softMax-=a,e.max64High=a}else{const t=e.min.map(Math.fround);e.min=e.min.map((i,r)=>i-t[r]),e.softMin=e.softMin.map((i,r)=>i-t[r]),e.min64High=t;const a=e.max.map(Math.fround);e.max=e.max.map((i,r)=>i-a[r]),e.softMax=e.softMax.map((i,r)=>i-a[r]),e.max64High=a}return e}const p={"vs:#main-start":`
